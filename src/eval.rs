...
fn evaluate_five(cards: [Card; 5]) -> HandValue {
    ...
    let mut freq = [0u8; 15];
    for &v in &vals { freq[v as usize] += 1; }
    ...
    if is_flush && (is_straight || is_wheel) {
        return if vals[0] == 14 && !is_wheel {
            HandValue { category: HandCategory::RoyalFlush, tiebreak: vec![] }
        } else {
            let top = if is_wheel { 5 } else { vals[0] };
            HandValue { category: HandCategory::StraightFlush, tiebreak: vec![top] }
        };
    }
...
fn best_hand(cards: &[Card]) -> HandValue {
    ...
    for i3 in i2+1..n-1 {
    for i4 in i3+1..n {
        let val = evaluate_five([cards[i0], cards[i1], cards[i2], cards[i3], cards[i4]]);
        if best.as_ref().map_or(true, |b| val > *b) { best = Some(val); }
    }}}}}
...
pub fn simulate(
    ...
    let mut base_deck: Vec<Card> = Suit::ALL.iter()
