...
fn eval7(seven: &[Card; 7]) -> u32 {
    ...
    if let Some(fs_idx) = suit_cnt.iter().position(|&c| c >= 5) {
        ...
        let is_wheel_flush = sf_top == 0
            && fv[..nf].contains(&14) && fv[..nf].contains(&5)
            && fv[..nf].contains(&4)  && fv[..nf].contains(&3)
            && fv[..nf].contains(&2);

        if sf_top == 14 { return pack(ROYAL_FLUSH, 0, 0, 0, 0, 0); }
        if sf_top > 0   { return pack(STR_FLUSH, sf_top, 0, 0, 0, 0); }
        if is_wheel_flush { return pack(STR_FLUSH, 5, 0, 0, 0, 0); }
